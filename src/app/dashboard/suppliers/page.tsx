...
export default function SuppliersPage() {
  ...
  const load = (q?: string) => {
    ...
    fetch(`/api/buyers${params}`).then((r) => r.json()).then((d) => setSuppliers(d.buyers ?? []));
  };
  ...
  const handleSave = () => {
    confirm(async () => {
      ...
      message: editingId
        ? `${t.suppliers.editSupplier}: "${form.name}"?`
        : `${t.suppliers.addSupplier}: "${form.name}"?`,
    });
  };
  ...
  const handleDelete = (s: Supplier) => {
    confirm(async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/buyers/${s.id}`, { method: "DELETE" });
        if (!res.ok) throw new Error((await res.json()).error);
        load(search);
      } catch (err) {
    ...
    }, { message: `Delete supplier "${s.name}"? (Only if no purchases exist)` });
  };

  return (
    <div>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-2">
        <h1 className="text-xl font-bold sm:text-2xl">{t.suppliers.title}</h1>
      ...
      <div className="mb-4">
        <Input placeholder={t.suppliers.searchByNameOrPhone} value={search} onChange={(e) => { setSearch(e.target.value); load(e.target.value); }} />
      </div>

      {showForm && (
        <div className="mb-6 grid grid-cols-1 gap-4 rounded-xl border border-[var(--border)] bg-[var(--card)] p-4 sm:p-6 md:grid-cols-3">
          <div className="md:col-span-3 font-semibold">{editingId ? t.suppliers.editSupplier : t.suppliers.addSupplier}</div>
      ...
      <div className="overflow-x-auto rounded-xl border border-[var(--border)]">
              ...
              <tr key={s.id} className="border-t border-[var(--border)]">
                ...
                <td className="px-4 py-3">
                  <div className="flex gap-2">
                    <Button size="sm" variant="outline" onClick={() => openEdit(s)} title={t.common.edit}>
                      <Pencil size={14} />
                    </Button>
                    <Button size="sm" variant="outline" onClick={() => handleDelete(s)} title={t.common.delete}>
        ...
        {suppliers.length === 0 && <p className="text-center text-gray-500 py-8">{t.common.noData}</p>}
      </div>
